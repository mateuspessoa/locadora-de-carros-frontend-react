...
const Aluguel = () => {
  ...
  const filtro = currentItens.filter((item) =>
    item.cliente.nome.toLowerCase().includes(searchLowerCase)
  );
  ...
  function tornarClienteInativo(id) {
    axios
      .post("http://localhost:8080/api/cliente/tornarinativo/" + id)
      .then((result) => {
        setAtualizar(result);
      });
  }
  ...
  function tornarClienteAtivo(id) {
    axios
      .post("http://localhost:8080/api/cliente/tornarativo/" + id)
      .then((result) => {
        setAtualizar(result);
      });
  }
  ...
  function tornarVeiculoIndisponivel(id) {
    axios
      .post("http://localhost:8080/api/veiculo/tornarindisponivel/" + id)
      .then((result) => {
        setAtualizar(result);
      });
  }
  ...
  async function tornarVeiculoDisponivel(id) {
    await axios
      .post("http://localhost:8080/api/veiculo/tornardisponivel/" + id)
      .then((result) => {
        setAtualizar(result);
      });
  }
  ...
  function confirmarDevolucao(id, idVeiculo, idCliente) {
    ...
    axios
      .post("http://localhost:8080/api/cliente/tornarinativo/" + idCliente)
      .then((result) => {
        setAtualizar(result);
      });
  }
  ...
  function confirmarPagamento(id) {
    axios
      .post("http://localhost:8080/api/aluguel/confirmarpagamento/" + id)
      .then((result) => {
        setAtualizar(result);
      });
  }
  ...
  async function calcularValorAluguel(idVeiculo) {
    await axios
      .get(`http://localhost:8080/api/veiculo/${idVeiculo}`)
      .then((result) => {
            ...
            {funcionarios?.map((funcionarioss) => (
              <>
                {funcionarioss.status == "Regular" && (
                  <option key={funcionarioss.id} value={funcionarioss.id}>
                    {funcionarioss.nome}
                  </option>
                )}
              </>
        ...
        <div className={styles.label}>
          ...
          >
            ...
            {clientes?.map((clientess) => (
              <option key={clientess.id} value={clientess.id}>
                {clientess.nome}
              </option>
            ))}
          </select>
            ...
            {veiculos?.map((veiculoss) => (
              <>
                {veiculoss.status == "Disponível" && (
                  <option key={veiculoss.id} value={veiculoss.id}>
                    {veiculoss.nome} | {veiculoss.placa}
                  </option>
                )}
              </>
            ))}
        ...
        <div className={styles.label}>
          ...
          <div className={styles.valorTotal}>
            <input
              type="text"
              name="valorAluguel"
              defaultValue={aluguel.valorAluguel || ""}
            />
            <button onClick={() => calcularValorAluguel(idVeiculo)}>
              Calcular Valor
            </button>
          </div>
        </div>
          ...
          <div className={styles.container_adcionar}>
            ...
            <div className={styles.container_search}>
              <input
                ...
                onClick={() => setItensPerPage(1000)}
                onBlur={() => setItensPerPage(5)}
              />
            ...
            <table>
              ...
              <tbody>
                {filtro?.map((alugueiss) => {
                  var data_formatada = new Date(
                    alugueiss.dataDevolucao
                  ).toLocaleDateString();
                    ...
                    <tr key={alugueiss.id}>
                      ...
                      <td className={styles.td_icones} data-label="Role">
                        {alugueiss.statusDevolucao == "Dentro do Prazo" && (
                          <>
                            <div
                              onClick={() => colocarEmAtraso(alugueiss.id)}
                              className={styles.tab_ico_2}
                            >
                              <AiOutlineClockCircle color="#edc204" />
                            </div>
                            <div
                              onClick={() =>
                                confirmarDevolucao(
                                  alugueiss.id,
                                  alugueiss.veiculo.id,
                                  alugueiss.cliente.id
                                )
                              }
                              className={styles.tab_ico_3}
                            >
                              <BsArrowBarDown color="#0000ff" />
                            </div>

                            <div
                              onClick={() => editar(alugueiss.id)}
                              className={styles.tab_ico_edit}
                            >
                              <AiFillEdit color="#00e7fc" />
                            </div>

                            <Link to={{ pathname: `/aluguel/${alugueiss.id}` }}>
                              <div className={styles.tab_ico_4}>
                                <BiDetail color="#fff" />
                              </div>
                            </Link>
                          </>
